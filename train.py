...
def train(
    ...
    """
    logger = Logger(log_dir=log_dir)  # Pass the log_file parameter
    ...
    for epoch in range(num_epochs):
        ...
        for b_data, b_labels in train_loop:
            ...
            train_loop.set_postfix(loss=loss, acc=np.sum(preds == b_labels) / b_data.shape[0])
        # Average metrics
        ...
        if val_loader:
            ...
            for b_data, b_labels in val_loop:
                ...
                # Update progress bar with current loss and accuracy
                val_loop.set_postfix(loss=loss, acc=np.sum(preds == b_labels) / b_data.shape[0])
