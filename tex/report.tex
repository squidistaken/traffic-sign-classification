...
\begin{itemize}
  ...
  \item \texttt{ToGrayscale}: Converts RGB images to grayscale using the ITU-R
        601-2 luma transform\footnote{The formula for each pixel value $L$ in the grayscale image is $L = \frac{1}{1000}(299 R + 587 G + 114 B)$, where $R$, $G$, and $B$ are the red, green, and blue pixel values, respectively.} implemented by the PIL library. We then convert back to RGB by duplicating the grayscale channel three times, as the network expects three channels.
...
During writing, the autocomplete feature was used to speed up formulation of sentences. The ideas and structure of the report are ours.
